...
const OVERLAY_DIR = path.join(OUTPUT_DIR, 'overlays');
const WORK_DIR = path.join(OUTPUT_DIR, 'work');

/**
 * The music bed. The cut is built to this track's tempo and phrase structure
 * (see timeline.js), so replacing it means re-deciding the bar counts there,
 * not just pointing this somewhere else.
 *
 * Deliberately not committed: it is a third-party track this repository has a
 * licence to *use*, not to redistribute. A checkout without it still builds —
 * build-video.js says so and cuts the film silent.
 */
const MUSIC_FILE = path.resolve(
  process.env.DEMO_MUSIC || path.join(__dirname, '..', 'Corporate Business Music.mp3'),
);
...
function ensureDirs() {
  for (const dir of [OUTPUT_DIR, RAW_DIR, SCENE_DIR, CARD_DIR, OVERLAY_DIR, WORK_DIR]) {
